...
- An ELISA equivalent test with 100% sensitivity and specificity with a 0-day test result delay @hiebertEvaluationDiagnosticAccuracy2021.
- An ELISA equivalent test with 100% sensitivity and specificity with a 14-day test result delay.

The time series of "test-positive cases" is the daily count of those tested cases that return a positive test result. Thus, for each measles (@fig-outbreak-schematic b) and non-measles noise (@fig-outbreak-schematic a) time series, we have 1 time series of outbreaks (@fig-outbreak-schematic a) and 5 possible time series of test-positive cases (@fig-outbreak-schematic c) (all clinically compatible cases, plus the 4 testing scenarios), which will include false positive and negative cases resulting from imperfect diagnostic tests, that can be used to trigger outbreak alerts.

== Triggering Alerts
<triggering-alerts>
#emph[#strong[Need to mention that an alert is any time the threshold is exceeded, regardless of the number of consecutive days of exceedance.];] #emph[#strong[Clearly state that 1 day of exceedance is and 30 days of consecutive exceedance are both treated as a single alert i.e., the same.];]

We define an "alert" as any contiguous string of 1 or more days where the 7-day moving average of the test-positive cases is greater than, or equal to, a pre-specified alert threshold, T. For each time series of test-positive cases, we calculate the percentage of alerts that are "correct", defined as any overlap of 1 or more days between the alert and outbreak period (@fig-outbreak-schematic c); this is analogous to the PPV of the alert system. Note that it is possible to have multiple alerts within a single outbreak, if the 7-day moving average of test positive cases drops below the threshold, and each would be considered correct. For all outbreaks that occur in the measles time series, we calculate the percentage for which at least 1 alert occurred between its start and end date (@fig-outbreak-schematic b); this is analogous to the sensitivity of the alert system. We also calculate the detection delay as the time from the start of an outbreak to the start of its first alert. If the alert starts before the outbreak and continues past the start date of the outbreak, this could be considered a correct alert with a negative delay i.e., an early warning triggered by a false positive.
