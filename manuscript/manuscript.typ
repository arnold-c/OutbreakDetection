...
== Methods
<methods>
We generated a stochastic SEIR model with importation to simulate true measles and non-measles sources of febrile rash (noise) daily incidence. We simulated the noise to have in-phase, out-of-phase, non-seasonal dynamics, with respect to the measles simulation. We also examined noise where each day were independent Poisson draws. For rapid diagnostic test (RDT) and enzyme-linked immunosorbent assay (ELISA) like tests, we varied the reporting rate, and the observed daily incidence threshold required to trigger an alert, and computed the: percentages of alerts that were correct (analogous to the positive predictive value), the percentage of outbreaks detected (analogous to the sensitivity), the delay between outbreak initiation and alert, and the number of (un)avoidable outbreak cases. Summarizing the percentage of alerts correct and outbreaks detected into an accuracy metric, we selected the "optimal threshold" for each combination of diagnostic test and reporting rate, presenting the summary statistics above.

== Results
<results>
The optimal threshold for each test increased monotonically as the percentage of clinic visited who were tested increased, though there are adjacent reporting rates that have identical thresholds due to the integer requirement for the daily threshold. With Poisson-only noise, similar outbreak detection accuracies could be achieved with imperfect RDT-like tests as with ELISA-like diagnostic tests (c. 93%), given moderately high testing rates. With larger delays (14 days) between the ELISA test procurement and result date, RDTs could outperform the ELISA. Similar numbers of unavoidable cases and outbreak alert delays could be achieved between the test types. With dynamical, in-phase, noise, however, the accuracy of ELISA scenarios was far superior to those achieved with RDTs (c. 93% vs.~73%). For dynamical noise, RDT-based scenarios typically favored more sensitive alert threshold than ELISA-based scenarios (at a given reporting rate), observed with lower numbers of unavoidable cases and detection delays.

== Conclusions
<conclusions>
The performance of an outbreak detection system is highly sensitive to the structure and the level of background noise. Under the assumption that the noise is relatively static over time (as influenza-like illness was early in the COVID-19 pandemic), RDTs can perform as well as ELISA in a surveillance system. However, when the noise is non-stationary, imperfect tests cannot overcome their accuracy limitations through higher testing rates.
...
In this paper we examine how the interaction between imperfect diagnostic tests affects the optimal outbreak alert method for the detection of measles outbreaks, in the context of a febrile rash surveillance system that detects and tests suspected measles cases i.e., an individual with fever and maculopapular (non-vesicular) rash (or who the healthcare worker suspects has measles) @essentialprogrammeonimmunizationepiimmunizationvaccinesandbiologicalsivbMeasles2018. Because measles symptoms are non-specific, it is important to account for non-measles sources of febrile rash e.g., rubella, parvovirus, varicella, etc., producing the potential for false positive results in the context of imperfect tests. Currently, measles outbreaks are declared on the basis of either suspected measles cases alone, the use of an enzyme-linked immunosorbent assay (ELISA) to detect the presence of measles-specific IgM antibodies, or a combination of the two, depending on the elimination status of the region, with countries nearer elimination increasing the use of ELISA diagnostic confirmation for suspected cases @essentialprogrammeonimmunizationepiimmunizationvaccinesandbiologicalsivbMeasles2018. Each of these detection systems have their flaws. Although clinical case definition is very fast and requires limited resources, providing a highly sensitive alert system, in the face of high "background noise" from non-measles sources of febrile rash relative to true measles prevalence, it can lead to great uncertainty in the alert system and low positive predictive value (PPV), i.e., the probability that an alert accurately captures an outbreak @hutchinsEvaluationMeaslesClinical2004. And while ELISA confirmation is the standard diagnostic test for measles surveillance @essentialprogrammeonimmunizationepiimmunizationvaccinesandbiologicalsivbMeasles2018, it is expensive due to its cold-chain and transport costs and delays resulting from the requirement to transfer samples from point of care to a central laboratory, leading to a slower and less sensitive (but more specific) alert system. A rapid diagnostic test offers the opportunity to provide a compromise between diagnostic accuracy and timeliness, and recent developments show encouraging signs in the field as well as in theory @warrenerEvaluationRapidDiagnostic2023@brownRapidDiagnosticTests2020.
...
== Model Structure
<model-structure>
We constructed a stochastic compartmental non-age structured SEIR model of measles and simulated using a modified Tau-leaping algorithm with a time step of 1 day, utilizing binomial draws to ensure compartment sizes remained positive valued @chatterjeeBinomialDistributionBased2005@gillespieApproximateAcceleratedStochastic2001. To stochastically reintroduce infections and cause recurring epidemics, commuter-style imports were added that are proportional to the size of the population and $R_0$ @keelingModelingInfectiousDiseases2008, and the transmission parameter ($beta_t$) is sinusoidal with a period of one year. $R_0$ was set to 16, with a latent period of 10 days and infectious period of 8 days. The population was initialized with 500\_000 individuals with Ghana-like birth and vaccination rates, and the final results were scaled up to the 2022 population size of Ghana (33 million). The full table of parameters can be found in @tbl-model-parameters. All simulations and analysis was completed in Julia version 1.10.3 @bezansonJuliaFreshApproach2017, with all code stored at #link("https://github.com/arnold-c/OutbreakDetection");.
...
To examine the sensitivity of the detection system to background noise, we layered the measles incidence timeseries with one of four noise time series: Poisson-only noise, or dynamical noise with rubella-like parameters that could be in- or out-of-phase or with independent seasonality to the measles dynamics. The total annual incidence of noise in each system was approximately 8 times the average annual measles incidence, with the dynamical noise systems also containing low levels of Poisson noise, reflecting Ghana-like dynamics. Despite the average incidence of noise in each scenario being the same, the relative proportion of measles to noise varies differently throughout the timeseries, exacerbating the effects of imperfect diagnostic tests that will produce higher rates of false positives and negatives than ELISA-like diagnostics. Imperfect tests are also more susceptible to incorrect results because of the varying prevalence throughout the timeseries, so the PPV of the system will not be static throughout the timeseries. To account for the stochasticity of the model, we simulated 100 time series for each scenario, each lasting 100 years, before summarizing the distributions of outbreak detection methods.

== Defining Outbreaks
<defining-outbreaks>
It is common to use expert review to define outbreaks when examining empirical data, but this is not feasible in a modeling study where tens of thousands of years are being simulated. To account for this, many studies only simulate a single outbreak within a timeseries (repeating this short stochastic simulation multiple times to ensemble results), define an outbreak as a period where $R_(upright("effective"))$ \> 1, or use a threshold of \> 2 standard deviations (s.d.) over the mean seasonal incidence observed in empirical data \[#cite(<sternAutomatedOutbreakDetection1999>, form: "prose");;; #cite(<jombartRealtimeMonitoringCOVID192021>, form: "prose");; #cite(<stolermanUsingDigitalTraces2023>, form: "prose");; #cite(<salmonMonitoringCountTime2016>, form: "prose");; #cite(<teklehaimanotAlertThresholdAlgorithms2004>, form: "prose");; #cite(<leclereAutomatedDetectionHospital2017>, form: "prose");\]. Each method has its uses, but to evaluate the performance of an outbreak detection system in an endemic region where multiple sequential epidemics are expected it is important to clearly define the bounds of the outbreak, which can only be achieved by one of these methods (2 s.d. \> mean). This, however, assumes strong seasonal forcing and regular periodicity of incidence to produce a smooth enough baseline, which is not present as countries near measles elimination status @grahamMeaslesCanonicalPath2019. Here we define a true measles outbreak as a region of the timeseries that meets the following three criteria:
...
Each of these is a necessary, but not sufficient, condition to the definition of an outbreak; all must be met. For example, a region may produce 1000 total cases within the lower and upper bounds defined by the daily incidence being $gt.eq$ 5 cases, but if this period only lasts for 20 days it would not be considered an outbreak as it would not be possible to mount a response to. The incidence of non-measles febrile rash (i.e., noise) does not affect the outbreak status of a region but does affect the alert status triggered by the testing protocol.

== Triggering Alerts
<triggering-alerts>
The use of alert thresholds is common in locations burdened by measles, where access to high performance computing is limited making nowcasting-style approaches to outbreak detection impractical. For this reason, we define an "alert" as any single day where the 7-day moving average of the daily incidence is greater than, or equal to, a threshold of X. To examine the interaction of test and surveillance systems we vary the alert threshold, X, between 1 and 15 cases per day, before computing the evaluation metrics and identifying the optimal threshold for each individual test. We also examined an alert method that required either the daily incidence or the 7-day moving average of daily incidence to exceed the alert threshold X (see supplemental results).

Each day, 60% of the measles and non-measles febrile rash cases visit the clinic for treatment, and Y% percentage of these clinic visits are tested, as all clinic visits are deemed to be suspected measles cases because they meet the clinical case definition. This percentage of clinic visits, Y, that are tested is varied between 10% and 60%, in 10% increments, for all combinations of diagnostic test (apart from the clinical case definition) and alert threshold, defining the "testing scenario". While a reporting rate of 6-48% is high, we are only simulating a population of 500\_000 individuals i.e., approximately 1.5% of Ghana’s population, which produces absolute incidence values that are consistent with observed reporting on the national scale. Each testing scenario uses one of the following tests:
...
#figure([
#box(image("./schematic-simulation_with-shade.png"))
], caption: figure.caption(
position: bottom, 
[
...
#figure([
#let optimal_thresholds = csv("optimal-thresholds-accuracy.csv")
