...
To examine the sensitivity of the detection system to background noise, we layered the measles incidence time series with one of four noise time series structures: Poisson-only noise; or dynamical noise with rubella-like parameters that could be in- or out-of-phase, or with independent seasonality to the measles dynamics. For Poisson-only noise, the number of non-measles febrile rash cases each day were independent draws from a Poisson distribution with mean $lambda$. For dynamical noise, we generated time series of cases from an SEIR model that matched the measles model in structure, but had $R_0 = 5$, mean latent period of 7 days, and mean infectious period of 14 days (@tbl-model-parameters). The seasonality for the dynamical noise was assumed to be in-phase with measles, anti-phase with measles (peak timing 6 months later), or non-seasonal. Only dynamical in-phase noise and Poisson-only noise are presented in the main text; the anti-phase and non-seasonal dynamical noise scenarios are presented in the supplement.

For each noise structure, we simulated five different magnitudes of noise ($Lambda$). $Lambda$ was calculated as a multiple ($c$) of the average daily measles incidence ($angle.l Delta I_M angle.r$): $Lambda = c dot.op angle.l Delta I_M angle.r upright("where") c in { 1 , 2 , 4 , 6 , 8 }$. For the Poisson-noise scenarios, independent draws from a Poisson distribution with mean $lambda = Lambda$ were simulated to produce the noise time series. For the dynamical noise scenarios, the vaccination rate at birth was set to 85.4%, 73.8%, 50.8%, 27.9%, and 5.0% to produce equivalent mean daily noise values (to within 2 decimal places): $Lambda = angle.l Delta I_R angle.r = c dot.op angle.l Delta I_M angle.r$.
...
#emph[#strong[Mention that the same model was used to define the noise as the outbreaks.];]
