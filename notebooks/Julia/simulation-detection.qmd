...
```{julia}
infection_rate(u, p, t) = p[1] * u[1] * u[2] / (u[1] + u[2] + u[3])      # β*X*Y / N
...
```{julia}
recovery_rate(u, p, t) = p[2] * u[2]     # γ*Y
...
```{julia}
X_death_rate(u, p, t) = p[3] * u[1]       # μ * X
...
```{julia}
Y_death_rate(u, p, t) = p[3] * u[2]       # μ * Y
...
```{julia}
Z_death_rate(u, p, t) = p[3] * u[3]       # μ * R
