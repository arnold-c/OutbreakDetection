...
tstep = 0.1/365
tspan = (0.0, tmax)
tlength = length(tspan[1]:tstep:tspan[2])
Random.seed!(1234);

init_states = [999.0, 10.0, 0.0, 0.0]
...
gamma = 365/5
epsilon = 100
p = (beta, sigma, gamma, mu, epsilon)
```

```{julia}
x = 0:0.01:2
fig = lines(x, 0.5 .* (sin.(2pi .* x) .+ 1))
vlines!(collect(0:1:2); color = (:red, 0.5), linestyle = :dash)
fig
```

```{julia}
# 0.5 .* (sin.(2π .* x) .+ 1)
expose_rate(u, p, t) = p[1] * 0.5 * (sin(2π * t) + 1) * u[1] * u[2]/ (u[1] + u[2] + u[3] + u[4])  # beta * ϕ * S * I / N
...
infec_rate(u, p, t) = p[2] * u[2]   # sigma * E
...
recov_rate(u, p, t) = p[3] * u[3]   # gamma * I
...
birth_rate(u, p, t) = p[4] * (u[1] + u[2] + u[3] + u[4]) # mu * N
...
prob = DiscreteProblem(init_states, tspan, p)
...
```{julia}
function run_sir_sim(; init_states, tspan, p)
    prob = DiscreteProblem(init_states, tspan, p)
    jump_prob = JumpProblem(
        prob, Coevolve(), gillespie_jumps..., save_positions = (false, false);
        dep_graph
        )
    jump_sol = solve(jump_prob, SSAStepper(), saveat = tstep)
    return jump_sol;
end
```
...
@showprogress for i in 1:nsims
    out_jump = run_sir_sim(init_states = init_states, tspan = tspan, p = p)
