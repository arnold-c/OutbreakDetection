...
The jump process is coupled to an SDE process that simulates seasonal background noise.
...
r = 1.0 - (s + i)
η = 0.01
u₀ = convert.(Float64, [N * s, N * i, N * r, N, η * N])
δt = 0.5
tlower = 0.0
tmax = 365.0 * 100
tspan = (tlower, tmax)
...
#%%
function background_ode!(du, u, p, t)
    return du[5] = 0.0
end

function background_noise!(du, u, p, t)
    return du[5] = 0.5
end
...
#%%
season_infec_prob = SDEProblem(background_ode!, background_noise!, u₀, tspan, p)
# test_sol = solve(season_infec_prob, SRIW1())

# DataFrame(test_sol)

# lines(test_sol.t, DataFrame(test_sol)[:, 6], label = "Noise", color = "purple")
...
#%%
sircolors = ["dodgerblue4", "firebrick3", "chocolate2", "purple", "green"]
labels = ["S", "I", "R", "N", "Noise"]

draw_sir_plot(
    season_infec_sol_df; colors = sircolors, labels = labels, annual = true
)
