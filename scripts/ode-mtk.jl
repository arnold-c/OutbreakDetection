...
D(S) ~ -β * S * I + μ * (I + R), D(I) ~ β * S * I - γ * I - μ * I,
    D(R) ~ γ * I - μ * R]
...
#%%
Jac = calculate_jacobian(de_simple)[(nac + 1):(nac + nic * nac),
    (nac + 1):(nac + nic * nac)]
...
eigen(
    substitute(
        calculate_jacobian(de_simple)[(nac + 1):(nac + nic * nac),
            (nac + 1):(nac + nic * nac)],
...
create_sir_all_sims_array!(;
    nsims = nsims, prob = gill_jump_prob, alg = SSAStepper(),
    δt = δt)
