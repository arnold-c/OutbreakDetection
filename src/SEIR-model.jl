...
function seir_mod_long(
    ...
    state_arr, change_arr, jump_arr = seir_mod(
        states, dynamics_params, time_params; retbetaarr = retbetaarr, type = type, seed = seed
    )

    state_long = permutedims(state_arr, (2, 1))
    change_long = permutedims(change_arr, (2, 1))
    jump_long = permutedims(jump_arr, (2, 1))

    return state_long, change_long, jump_long
end
...
function seir_mod_long!(
    state_arr, change_arr, jump_arr, states, rates, dynamics_params, time_params;
    type = "stoch",
    seed = 1234,
)
    Random.seed!(seed)
    ...
    for (i, t) in pairs(time_params.trange)
        ...
        seir_mod_long_loop!(
            state_arr, change_arr, jump_arr, rates, i, dynamics_params, time_params, t;
            type = type,
        )
    end
...
function seir_mod_long_loop!(
    state_arr, change_arr, jump_arr, rates, i, dynamics_params, time_params, t;
    type = type,
)
    # Calculate beta for current time
    beta_t = calculate_beta_amp(dynamics_params.beta_mean, dynamics_params.beta_force, t)

    # Calculate the rates of each event
    @views rates[1] = beta_t .* state_arr[i - 1, 1] .* state_arr[i - 1, 2]
    @views rates[2] = dynamics_params.sigma .* state_arr[i - 1, 2]
    @views rates[3] = dynamics_params.gamma .* state_arr[i - 1, 3]
    @views rates[4] = dynamics_params.mu .* state_arr[i - 1, 5]
    @views rates[5] = dynamics_params.mu .* state_arr[i - 1, 1]
    @views rates[6] = dynamics_params.mu .* state_arr[i - 1, 3]
    @views rates[7] = dynamics_params.mu .* state_arr[i - 1, 2]
    @views rates[8] = dynamics_params.mu .* state_arr[i - 1, 4]
    @views rates[9] = dynamics_params.epsilon .* state_arr[i - 1, 5] ./ dynamics_params.R_0
