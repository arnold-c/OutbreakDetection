...
function seir_mod_long!(
    ...
    for (i, t) in pairs(time_params.trange)
        ...
        seir_mod_long_loop!(
            state_arr, change_arr, jump_arr, i, dynamics_params, time_params, t;
            type = type,
        )
    end
...
function seir_mod_long_loop!(
    ...
    change_arr[i, 3] = jump_arr[i, 2] - jump_arr[i, 3] - jump_arr[i, 7]
    change_arr[i, 4] = jump_arr[i, 3] - jump_arr[i, 8]
    change_arr[i, 5] = sum(change_arr[i, 1:4])

    # Check that the change in each state does not result in a negative state,
    # and if it is, set the change to the negative of the current state (i.e.
    # set the new state value to 0)
    for state in axes(state_arr, 2)
        if state_arr[i - 1, state] + change_arr[i, state] < 0
            change_arr[i, state] = -state_arr[i - 1, state]
        end
    end

    @. state_arr[i, :] = state_arr[i - 1, :] + change_arr[i, :]
