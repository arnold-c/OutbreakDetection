...
function line_plot(
    ...
    @assert plotformats isa Tuple
    mkpath(plotdirpath)

    normalized_plotformats = map(plotformats) do format
        format_string = lowercase(String(format))
        startswith(format_string, ".") &&
            (format_string = format_string[2:end])

        format_string
    end

    plotpaths = map(normalized_plotformats) do format
        joinpath(plotdirpath, "$plotname.$format")
    end

    if save_plot && !force && all(isfile, plotpaths)
        return nothing
    end
    ...
    result_matrix, unique_noise_types =
        reshape_optimization_results_to_matrix(filtered_results)
        ...
        map(
            ...
            if outcome == :alert_threshold
                ...
                return [
                    ...
                    LineElement(; color = colors[original_idx], linestyle = linestyle),
                ]
            end
        end,
    ...
    if save_plot
        for plotpath in plotpaths
            if force || !isfile(plotpath)
                Makie.save(plotpath, fig; size = size, pt_per_unit = pt_per_unit)
            end
        end
    end
